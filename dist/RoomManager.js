...
import { createDeck, shuffleDeck, getVazaWinner, } from "./shared/cards.js";
...
export class RoomManager {
    ...
    joinRoom(code, socketId, nickname, team) {
        ...
        if (teamCount >= 2)
            return { error: `Time ${team} já está cheio` };
    ...
    playCard(roomId, socketId, cardIndex) {
        const room = this.rooms.get(roomId);
        if (!room)
        ...
        if (room.game.currentVaza.length === 4) {
            ...
            let vazaWinner = "draw";
            if (winner) {
                const winnerPlayer = room.game.players.find((p) => p.id === winner.playerId);
                if (winnerPlayer) {
            ...
            if (room.game.vazaScores.A >= 2 || room.game.vazaScores.B >= 2) {
                ...
                if (vazaWinner !== "draw" && winner) {
                    const winnerIndex = room.game.players.findIndex((p) => p.id === winner.playerId);
                    if (winnerIndex !== -1) {
    ...
    requestTruco(roomId, socketId) {
        ...
        if (currentLevel === 1)
            ...
            return { success: false, error: "Aposta já está no máximo" };
        room.game.trucoRequest = { from: socketId, level: nextLevel };
        return { success: true, level: nextLevel };
    }
    respondTruco(roomId, socketId, accept) {
        const room = this.rooms.get(roomId);
        if (!room)
            return { success: false, error: "Sala não encontrada" };
        if (!room.game.trucoRequest)
            return { success: false, error: "Nenhum pedido de truco pendente" };
        ...
        if (accept) {
            ...
            const playerWhoRequested = room.game.players.find((p) => p.id === request.from);
            if (!playerWhoRequested)
    ...
    respondMãoDeOnze(roomId, socketId, accept) {
        const room = this.rooms.get(roomId);
        if (!room)
            return { success: false, error: "Sala não encontrada" };
        if (!room.game.mãoDeOnze || room.game.mãoDeOnze.status !== "pending") {
    ...
    reconnectPlayer(socketId, roomId) {
        const room = this.rooms.get(roomId);
        if (!room)
            return null;
